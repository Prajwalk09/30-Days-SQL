...
-- price in each country, return product name, price and country
...
rank() over (partition by s.country order by p.price desc) as rankings
from products as p
...
from customers as c 
join transaction as t on t.customer_id = c.customer_id
